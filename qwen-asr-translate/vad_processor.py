...
class VADProcessor:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        self.state = None  # RNN 狀態
        
        # VAD 參數
        self.threshold = 0.35  # 語音閾值（降低到 0.35，更敏感以捕捉小聲說話）
        self.min_speech_duration = 0.2  # 最小語音長度（秒）- 降低到 0.2 秒，捕捉短語
        self.min_silence_duration = 1.2  # 最小靜音長度（秒）- 增加到 1.2 秒，避免切斷慢速說話或停頓
    ...
    def _load_model(self):
        ...
        print(f"[OK] VAD model loaded: {self.model_path}")
    ...
    def detect_speech_segments(self, audio: np.ndarray, sample_rate: int = 16000) -> List[Tuple[float, float]]:
        ...
        # 分幀處理
        frame_size_samples = int(self.sample_rate * 0.04)  # 40ms
        segments = []
        ...
        for i in range(0, len(audio), frame_size_samples):
            ...
            # 執行 VAD 推理
            prob = self.detect_speech(frame, self.sample_rate)
